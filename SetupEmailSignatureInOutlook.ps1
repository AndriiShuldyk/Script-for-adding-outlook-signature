...
"@ -ForegroundColor Cyan
  ...
  while ($true)
  {
    ...
    {   
      ...
      $Output = $Output + "to $(($UsersCollection.UserPrincipalName).Count) Users in the tenant..."
      Write-Host $Output
    }
    elseif ($ImportUsersType -eq 2)
    {
      ...
      while ($true)
      {
        try
        {
          ...
          if ('UserPrincipalName' -in $Headers -and ($UsersCollection.UserPrincipalName).Count -ge 1)
          {
            $Output = $Output + "to $(($UsersCollection.UserPrincipalName).Count) users in the CSV file..."
            Write-Host $Output
            break;
          }
          else
...
function Get-HTMLContent
{
  ...
  while ($true)
  {
    try
    {
      $HTMLcontent = Get-Content -Path $Path -Raw -Encoding UTF8 -ErrorAction Stop
